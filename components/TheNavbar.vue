  ...
  >
    <div class="max-w-6xl mx-auto px-6 h-16 flex items-center justify-between">
      ...
      <a href="#" class="flex items-center gap-2.5">
        <div class="w-8 h-8 bg-editus-600 rounded-lg flex items-center justify-center">
          <svg width="18" height="18" viewBox="0 0 18 18" fill="none">
            ...
            <rect x="2" y="7.5" width="6" height="4" rx="1" fill="white" opacity="0.65"/>
            <rect x="2" y="13" width="6" height="3" rx="1" fill="white" opacity="0.35"/>
            <rect x="10" y="2" width="6" height="14" rx="1" fill="white" opacity="0.12"/>
      ...
      <nav class="hidden md:flex items-center gap-8" aria-label="Menu principal">
        <a href="#problema" class="text-sm text-white/60 hover:text-white transition-colors">Problema</a>
        <a href="#como-funciona" class="text-sm text-white/60 hover:text-white transition-colors">Como funciona</a>
        <a href="#features" class="text-sm text-white/60 hover:text-white transition-colors">Funcionalidades</a>
        <a href="#faq" class="text-sm text-white/60 hover:text-white transition-colors">FAQ</a>
      </nav>

      <!-- CTA -->
      <a href="#waitlist" class="btn-primary text-sm py-2" @click="onCtaClick">
        Acesso antecipado
...
onMounted(() => {
  if (process.client) window.addEventListener('scroll', onScroll, { passive: true })
})
onUnmounted(() => {
