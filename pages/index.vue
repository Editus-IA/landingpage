<template>
  <div>
    <TheNavbar />
    <main id="conteudo-principal" class="relative">
      ...
      <template v-for="section in sectionOrder" :key="section">
        ...
        <ClientOnly        v-else-if="section === 'dashboard'">
          <DashboardPreview />
          <template #fallback><div class="py-24 bg-white" /></template>
        </ClientOnly>
        <FAQSection        v-else-if="section === 'faq'"
          :cta-copy-variant="ctaCopyVariant"
          :button-color-variant="buttonColorVariant"
        />
        <!-- CTA Section: renderizada apenas na variante 'active' -->
        <CTASection        v-else-if="section === 'cta-section' && ctaSectionVariant === 'active'"
          :button-color-variant="buttonColorVariant"
        />
        <WaitlistForm      v-else-if="section === 'waitlist'"
          :cta-copy-variant="ctaCopyVariant"
          :button-color-variant="buttonColorVariant"
