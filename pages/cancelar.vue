    ...
    <div class="max-w-md w-full text-center">
      ...
      <template v-if="done">
        <div class="w-14 h-14 bg-editus-800/60 border border-editus-700/40 rounded-2xl flex items-center justify-center mx-auto mb-8">
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none" class="text-white/60">
            <path d="M5 12l5 5 9-9" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/>
          </svg>
        </div>
        <h1 class="text-2xl font-semibold text-white tracking-tight mb-3">Inscrição cancelada</h1>
        ...
        <NuxtLink to="/" class="text-sm text-editus-400 hover:text-editus-300 transition-colors">Voltar ao início</NuxtLink>
      </template>

      <template v-else-if="invalid">
        <h1 class="text-2xl font-semibold text-white tracking-tight mb-3">Link inválido</h1>
        <p class="text-white/50 text-sm leading-relaxed mb-8">
          Este link de cancelamento é inválido ou já foi utilizado.
          Para remover seus dados, entre em contato com
          <a href="mailto:privacidade@editus.com.br" class="text-editus-400 hover:text-editus-300 transition-colors">privacidade@editus.com.br</a>.
        </p>
        <NuxtLink to="/" class="text-sm text-editus-400 hover:text-editus-300 transition-colors">Voltar ao início</NuxtLink>
      </template>

      <template v-else>
        <h1 class="text-2xl font-semibold text-white tracking-tight mb-3">Cancelar inscrição</h1>
        <p class="text-white/50 text-sm leading-relaxed mb-8">
        ...
        <br />
        <NuxtLink to="/" class="text-sm text-white/40 hover:text-white/60 transition-colors">Manter minha inscrição</NuxtLink>
      </template>
